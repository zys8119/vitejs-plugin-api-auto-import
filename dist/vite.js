...
function transformFile(config, apiDirPath, mainFilePath, resolveAliasName) {
  ...
  const files = (0, import_fast_glob.sync)(`${(0, import_path.resolve)(apiDirPath, "**/*.ts").replace(/\\/g, "/")}`).filter((e) => !e.includes(mainFilePath) && (Object.prototype.toString.call(config.exclude) === "[object RegExp]" ? !config.exclude.test(e) : true));
  ...
  const resolversMap = resolvers.map((e, k) => {
    ...
    const name = resolveStr.match(/resolve\s*\((.*)\)/im)[1];
    const nameHash = `NAMEHASH_${[...name].map((e2) => e2.charCodeAt(0).toString(16)).join("")}_${(Date.now() + k).toString(16)}`;
    try {
      return {
        import: `
import * as ${nameHash} from "${e.from}"`,
        const: `
const ${name} = ${nameHash}`,
        data: resolveStr.replace(/resolve\s*\(.*?\)\s*\{\s*\n|\s*\}$/g, "").replace(/return\s*\{|};$/g, "").trim()
      };
    } catch (e2) {
      return {
        import: "",
...
${resolversMap.map((e) => e.data).join(",")},`);
  ...
  const templateStr = (0, import_lodash.template)((0, import_fs_extra.readFileSync)((0, import_path.resolve)(__dirname, "../src/template.ts"), "utf-8"))(templateData);
  (0, import_fs_extra.writeFileSync)((0, import_path.resolve)(apiDirPath, config.outFile), (0, import_js_beautify.js_beautify)(templateStr));
}
function autoApi(options) {
  const config = (0, import_lodash.merge)({
    name: "$apis",
    dir: "api",
